...
{{/*
三参数 dig 的等价实现:sprig 的 dig 函数需 Helm 3.2+ 才自带,
三星环境 Helm 3.0.3 没有,这里用 get/kindIs/hasKey 还原同样语义
(任一层键缺失或非 map 时返回默认值,键存在时返回原值)。
用法: {{ include "kube-prometheus-stack.dig3" (list "规则名" "字段" "默认值" .Values.customRules) }}
*/}}
{{- define "kube-prometheus-stack.dig3" -}}
{{- $keyA := index . 0 -}}
{{- $keyB := index . 1 -}}
{{- $default := index . 2 -}}
{{- $dict := index . 3 -}}
