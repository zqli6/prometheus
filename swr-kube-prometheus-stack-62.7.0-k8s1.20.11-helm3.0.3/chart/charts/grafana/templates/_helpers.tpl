...
{{/*
Generates a random password. The original version used `lookup` to reuse the
password of an existing Secret, but `lookup` requires Helm 3.1+ and is not
compatible with Helm 3.0.x (Samsung cluster). kube-prometheus-stack always
sets adminPassword (prom-operator), so this branch is not hit in practice.
*/}}
